...
screen::screen(const obby::buffer& buffer)
 : m_buffer(buffer), m_scroll_x(0), m_scroll_y(0)
{
	initscr();
	cbreak();
	noecho();
...
{
	move(y - m_scroll_y, x - m_scroll_x);
}

void screen::on_insert(obby::position position, const std::string& text)
{
	unsigned int cursor_x = get_cursor_x();
	unsigned int cursor_y = get_cursor_y();
	obby::position cursor = m_buffer.coord_to_position(cursor_x, cursor_y);
	...
	if(position <= cursor) cursor += text.length();
	out << "Cursor after insert: " << cursor << std::endl;
	m_buffer.position_to_coord(cursor, cursor_x, cursor_y);
...
void screen::on_erase(obby::position from, obby::position to)
{
	...
	out << "Cursor after delete: " << cursor << std::endl;
	m_buffer.position_to_coord(cursor, cursor_x, cursor_y);
	set_cursor(cursor_x, cursor_y);
}
...
/*	unsigned int x, y;
	m_buffer.position_to_coord(record.get_position(), x, y);
//	printw("Coord at %d: %d/%d\n", record.get_position(), x, y);
	move(y, x);
	clrtoeol();
	mvprintw(y, x, m_buffer.get_line(y).c_str() + x);*/
}
...
/*	unsigned int x, y;
	m_buffer.position_to_coord(record.get_begin(), x, y);
//	printw("Coord at %d: %d/%d\n", record.get_begin(), x, y);
	move(y, x);
	clrtoeol();
	mvprintw(y, x, m_buffer.get_line(y).c_str() + x);*/
}
